...
**派发前必做**：列出每个 Developer 的文件路径 → 检查交集。B 依赖 A 的产出？→ 串行 · 改同一文件？→ 串行 · 都不是 → 并行（`isolation="worktree"`，cherry-pick 合并）
...
按项目类型选择同步策略（详见 `resources/dispatch-rules.md`）。所有类型必做：测试报告 + CLAUDE.md 数据同步。文档后执行全量扫描。
...
- `resources/claude-md-template.md` — CLAUDE.md 模板。Step -1 自动集成
