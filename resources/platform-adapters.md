...
| Research | `Agent(description="Research: 调研<domain>", subagent_type="Explore")` |

工具：`Read, Write, Edit, Bash, Grep, Glob, WebSearch, WebFetch`。Research Agent 仅使用 `WebSearch, WebFetch`（只读外部搜索）。并行 Developer 使用 `isolation="worktree"`。
...
| RE | 根据测试失败日志修复 |
| Research | 对话中描述搜索方向，使用 `@web` 搜索后总结发现 |
