...
```markdown
## 设计约束 —— 必须满足（来自 Step 0.5 Research）
- PCI-DSS 4.0：PAN 必须 AES-256-GCM 加密存储。来源 https://pcisecuritystandards.org/...
  满足方式：[Architect 在此说明]
- 支付幂等性：所有写操作 API 必须接受 Idempotency-Key 头。来源 https://stripe.com/docs/...
  满足方式：[Architect 在此说明]
- 金额存储：所有金额字段为 integer（单位：分），禁止 float/double。来源 https://stripe.com/docs/currencies#zero-decimal
  满足方式：[Architect 在此说明]

## 推荐模式 —— 建议采纳（来自 Step 0.5 Research）
- Tokenization 模式：用 token 替代真实卡号。来源 https://...
  采纳 是/否，理由：[Architect 在此说明]
- Outbox Pattern：确保数据库写入和消息发布的原子性。来源 https://microservices.io/...
  采纳 是/否，理由：[Architect 在此说明]
```
...
压缩前（~45 tokens）：
  "设计约束：所有金额字段必须使用 integer 类型，单位为分（cent）。
   禁止使用 float 或 double 类型表示货币值。数据库列类型为 BIGINT。"

压缩后（~12 tokens，压缩率 ~73%）：
  "金额:int(分) | DB:BIGINT | 禁:float/double"

规则：
  - 硬约束（standard）→ key:value，保留具体数值
  - 模式建议（pattern）→ "推荐:[方案名](来源关键词)"
  - 反模式（anti-pattern）→ "禁:[关键词]"
  - 参考（reference）→ "参考:[链接关键词]"
  - 来源 URL 保留但不压缩（Agent 需自行验证）
```
...
| 原始 | 键值压缩后 |
|------|------|
| PAN 必须以 AES-256-GCM 加密存储，密钥由 KMS 管理 | `PAN:AES-256-GCM \| 密钥:KMS` |
| 所有写操作 API 必须接受 Idempotency-Key 头，数据库需唯一索引 | `幂等:Idempotency-Key头 \| DB:唯一索引(payment_id+idempotency_key)` |
| 禁止在日志中输出 CVV、PIN、完整 PAN | `禁日志:CVV/PIN/fullPAN` |
| 推荐使用 Tokenization 模式替代直接存储卡号 | `推荐:Tokenization(Stripe)` |

**压缩率**：通常 50-70%，具体取决于原始内容的 prose 密度。标准/反模式类压缩率最高，模式建议类较低。

**达标判定**：压缩后重新计算 tokens。若 ≤50% 上限 → 注入压缩版。若仍 >50% → 进入第二级。
...
同 category 内进一步排序：
  1. sourceVerified=false 优先丢弃
  2. 与当前 Agent 职责关联最弱的优先丢弃
     （如 QA 桶中的 pattern 发现，对 QA 价值低于对 Architect 的价值）
```

**操作**：
1. 计算需要丢弃的 tokens 数
2. 从 reference 类开始逐条丢弃，每丢一条重新计算
3. 丢弃的条目记录到 Knowledge Brief 中："以下发现因 token 预算未注入 [Agent 名]：[条目列表]"

**达标判定**：若 ≤50% → 注入截断版。若 reference + anti-pattern 全部丢弃后仍 >50% → 进入第三级。
...
**压缩率**：通常 40-60%，取决于约束密度。纯硬约束列表压缩空间小，软建议多则压缩空间大。

**最后手段**：三级压缩后仍超标 → PM 硬截断到上限，并在复盘（Step 7）中记录"本次 [Agent 名] 注入内容因超额被截断，以下发现未经充分传达：[列表]"。
...
| 级别 | 执行者 | 原因 |
|:---:|------|------|
| 第一级 | 轻量 LLM | 机械文本变换，不需要理解语义 |
| 第二级 | PM（规则） | 按优先级表直接丢弃，纯规则操作 |
| 第三级 | 轻量 LLM | 摘要生成需要语言能力但不需要深度推理 |
| 审核 | PM | 每级压缩后 PM 过一眼确认约束没丢 |

---

### 3.4 故障分级应对

当压缩管线中的某一环节失败时，按故障级别逐级降级，确保 Agent 至少能拿到有效输入。

#### 故障分级表

| 级别 | 场景 | 应对 | 执行者 |
|:---:|------|------|:---:|
| **L1** | 轻量 LLM 不可用（调用失败/超时/无轻量模型） | 跳过第一级，直接从第二级（PM 规则截断）开始。第三级同样跳过，直接硬截断 | PM（自动） |
| **L2** | 某级压缩导致硬约束语义丢失 | PM 审核时发现 → 回退到上一级结果，改为丢弃该 Agent 的全部软建议（pattern）来腾空间，硬约束原样保留 | PM |
| **L3** | 三级耗尽 + 硬截断后硬约束大量丢失 | PM 判断丢失约束中是否含 CRITICAL 级别。是 → 通知 Leader。否 → 注入截断版 + 记录复盘 | PM |
| **L4** | 所有路径均失败，无法生成任何有效注入 | **降级注入**（见下方） | PM |

#### L4 降级注入（最后兜底）

当所有压缩手段均失败，PM 放弃完整注入，替换为**最简降级提示词**（≤100 tokens）：

```markdown
## 上下文调研摘要（降级模式——完整注入因压缩失败未加载）
本次调研覆盖 [关注面列表]，发现 [N] 条约束/[M] 条推荐模式/[K] 条反模式。
核心约束：[仅 standard 类 finding 的标题，一行一条，不展开]
请在执行时结合你的领域知识关注以上方面。
完整 Knowledge Brief 见 [文档路径]。
```

**降级注入规则**：
- standard 类仅保留标题，不展开细节（Agent 自身知识可补位）
- pattern / anti-pattern / reference 全部丢弃
- 必须附 Knowledge Brief 文档路径（Agent 需要时可自行读取）
- 降级注入不经过压缩管线，PM 直接手写
- 记录到 Knowledge Brief + 复盘："[Agent 名] 因压缩全流程失败触发降级注入，[N] 条发现未充分传达"

#### PM 审核要点（每级压缩后必查）

- [ ] 硬约束的**具体数值**是否保留？（金额单位、加密算法、超时阈值等）
- [ ] 硬约束的**禁止项语义**是否保留？（"禁止 X" 不能变成 "注意 X" 或 "避免 X"）
- [ ] source URL 是否完好无损？
- [ ] 压缩后内容对目标 Agent 是否仍**可理解**？（不是乱码缩写或孤立符号）
- [ ] 第二级丢弃后，剩余内容是否仍覆盖所有关注面？

---

### 3.5 各 Agent 压缩阈值速查

| Agent | 最大注入 | 50% 阈值 | 触发示例（超过此值即压缩） |
|------|:---:|:---:|------|
| Architect | 500 | 250 | 5 条 prose 发现 (~50/条) |
| SL | 350 | 175 | 3 条长 prose 发现 |
| Reviewer #1 | 250 | 125 | 2 条长 prose 发现 |
| Reviewer #2 | 250 | 125 | 2 条长 prose 发现 |
| Reviewer #3 | 250 | 125 | 2 条长 prose 发现 |
| QA | 300 | 150 | 3 条 prose 发现 |
| Developer | 200 | 100 | 2 条 prose 发现 |
| RE | 150 | 75 | 1 条长 prose 发现 |
