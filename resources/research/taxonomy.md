...
3. **未命中处理**：分三种情况——
   ...
   **3a. 仅映射表未命中**（项目类型+业务领域组合不在映射表中，但分类体系可覆盖）：
   - 仍执行关键词提取，按关键词命中关注面搜集
   - 告知 Leader："本次组合未命中映射表，已基于关键词补充"
   - 记录到 `resources/research/taxonomy-extended.md`

   **3b. 业务领域或项目类型未命中**（第一层或第二层无匹配）：
   - 关键词提取照常执行
   - PM 将新类型/领域加入 taxonomy-extended.md 候选列表
   - 告知 Leader："[项目类型] 或 [业务领域] 为新增，已记录候选。建议搜索方向以关键词为准"

   **3c. 三层全未命中**（项目类型、业务领域、技术关注面均无匹配）：
   - PM 坦承 Leader："本次需求领域（[描述]）超出 DevFlow 现有知识分类体系。将切换为探索模式：以你的需求描述为搜索关键词，不做预分类限定。"
   - **探索模式**：Research Agent 使用 Leader 的需求原文作为搜索词，不限定 A-J 关注面
   - 搜索结果按"新领域"标注，PM 提取关键模式后提议新增分类或关注面
   - 记录到 taxonomy-extended.md §三层全未命中
   - 不阻塞流程——探索模式下的搜索结果仍注入下游提示词，但标注"领域外参考"

   **所有未命中情况的底线**：默认搜集安全(B)和架构(A)两个面保底。
...
每次 Step 7 复盘时，PM 检查本次调研的覆盖效果。同时检查扩展表是否需要整理——详见 `resources/research/taxonomy-reorganization.md`。

1. **未命中评估**：检查 taxonomy-extended.md 中的未命中记录——是否有应晋升的组合？是否有候选类型/领域已满足晋升条件？
2. **新增映射**：本次调研中发现了哪些映射表中没有的搜索方向？记录到 taxonomy-extended.md 候选列表。
3. **修正映射**：映射表中是否有过时或不准确的条目？如有直接修正。
...
所有未命中记录、候选类型/领域、晋升记录 → 见 `resources/research/taxonomy-extended.md`。
